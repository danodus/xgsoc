...
#define CS_ENABLE(_ctx_)  { _ctx_->sd_cs = CS_BIT; MEM_WRITE(SD_CARD, _ctx_->sd_cs); }
#define CS_DISABLE(_ctx_) { _ctx_->sd_cs = 0; MEM_WRITE(SD_CARD, _ctx_->sd_cs); }
...
static uint8_t spi_transfer(sd_context_t *ctx, uint8_t mosi)
{
    ...
    for (int i = 0; i < 8; ++i) {
        ...
        MEM_WRITE(SD_CARD, ctx->sd_cs | b);
        MEM_WRITE(SD_CARD, ctx->sd_cs | b | SCLK_BIT);
